  ...
  vm.perform(env => {
    ...
    try {
      ...
      if (offsetCopiedMethods === -1) {
        // Layout changed (seen on Android 16): there is no u16 matching the methods array length.
        // 0 is an impossible offset (it's the klass_ header), so it is used as a sentinel telling
        // the native side to iterate over the whole methods array instead of only the declared ones.
